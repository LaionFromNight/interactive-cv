...
export function ArchitectBriefTemplate({
  cv,
  options,
  styles,
}: {
  ...
  const selectedProjects = publicProjects.slice(0, 6);
  const remainingProjects = publicProjects.slice(6);
      ...
      <Page size="A4" style={styles.page} wrap={false}>
        ...
        <View style={styles.body}>
          <View style={styles.sidebar}>
            ...
            {cv.person.profiles?.length ? (
              <View style={styles.sidebarBlock}>
                <Text style={styles.sidebarTitle}>Links</Text>

                {cv.person.profiles.map((profile) => (
        ...
        <Footer
          ...
          consentText={remainingProjects.length ? null : consentText}
          styles={styles}
        />
      </Page>
